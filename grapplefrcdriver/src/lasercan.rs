...
use grapple_frc_msgs::{
    can::{CANId, CANMessage, FragmentReassembler},
    grapple::{
        lasercan::{LaserCanMessage, LaserCanRoi, LaserCanRoiU4, LaserCanStatusFrame},
        GrappleDeviceMessage, DEVICE_TYPE_DISTANCE_SENSOR, MANUFACTURER_GRAPPLE,
    },
    ManufacturerMessage, Message, Validate,
};
...
use crate::{
    hal_safe_call, with_err, COptional, HAL_CAN_ReceiveMessage, HAL_CAN_SendMessage,
    JNIResultExtension, HAL_CAN_SEND_PERIOD_NO_REPEAT,
};
...
impl NativeLaserCan {
    ...
    pub fn send(&mut self, msg: LaserCanMessage) -> anyhow::Result<()> {
        let msg = Message::new(
            self.can_id,
            ManufacturerMessage::Grapple(GrappleDeviceMessage::DistanceSensor(msg)),
        );
        self.send_ll(msg)
    }

    pub fn spin_once(&mut self) {
        let mut message_id: u32 = CANId {
            manufacturer: MANUFACTURER_GRAPPLE,
            device_type: DEVICE_TYPE_DISTANCE_SENSOR,
                    ...
                    Some((_, msg)) => match msg {
                        CANMessage::Message(msg) => match msg.msg {
                            ManufacturerMessage::Grapple(GrappleDeviceMessage::DistanceSensor(
                                grapple_frc_msgs::grapple::lasercan::LaserCanMessage::Status(
                                    status,
                                ),
                            )) => {
...
impl LaserCanDevice {
    pub fn new(can_id: u8) -> Self {
        Self {
            backend: Box::new(NativeLaserCan::new(can_id)),
        }
    }
}
...
fn get_handle<'local>(env: &mut JNIEnv<'local>, inst: JObject<'local>) -> *mut LaserCanDevice {
    let handle = env
        .get_field(inst, "handle", "Lau/grapplerobotics/LaserCan$Handle;")
        .unwrap()
        .l()
        .unwrap();
...
pub extern "system" fn Java_au_grapplerobotics_LaserCan_free<'local>(
    ...
    unsafe {
        drop(Box::from_raw(handle as *mut LaserCanDevice));
    }
}
...
pub extern "system" fn Java_au_grapplerobotics_LaserCan_status<'local>(
    ...
    match status {
        ...
        Some(status) => {
            let cls = env
                .find_class("au/grapplerobotics/LaserCan$RegionOfInterest")
                .unwrap();
            let roi = env
                .new_object(
                    cls,
            ...
            let cls = env
                .find_class("au/grapplerobotics/LaserCan$Measurement")
                .unwrap();
            env.new_object(
                cls,
                "(IIIZILau/grapplerobotics/LaserCan$RegionOfInterest;)V",
                &[
...
pub extern "system" fn Java_au_grapplerobotics_LaserCan_setTimingBudget<'local>(
    ...
    unsafe {
        (*lc)
            .set_timing_budget(budget as u8)
